...
fluxo_controle:
    ...
    | TK_PR_IF '(' expressao ')' bloco_comandos TK_PR_ELSE bloco_comandos { 
        ...
        ILOCOperation *label1_op = iloc_operation_create("nop", NULL, NULL, NULL, label1);
        ILOCOperation *label2_op = iloc_operation_create("nop", NULL, NULL, NULL, label2);
        ILOCOperation *label3_op = iloc_operation_create("nop", NULL, NULL, NULL, label3);

        ILOCOperationList *block_code = NULL;
        ILOCOperation *jump_op = NULL;
        if ($5 != NULL) { 
            asd_add_child($$, $5); 
            block_code = $5->code;
            jump_op = iloc_operation_create("jumpI", label3, NULL, NULL, NULL);
        } 

        ILOCOperationList *else_code = NULL;
        if ($7 != NULL) { 
            asd_add_child($$, $7);
            else_code = $7->code;
        }
...
expressao:
    ...
    | expressao TK_OC_OR expressao_precedencia_6 { 
        ...
        $$->code = iloc_list_concat(iloc_list_concat($1->code, $3->code), iloc_list_create_node(op));
    }
    ;
...
expressao_precedencia_3:
    ...
    | expressao_precedencia_3 '-' expressao_precedencia_2 { 
        ...
        $$->code = iloc_list_concat(iloc_list_concat($1->code, $3->code), iloc_list_create_node(sub_op));
    }
    ;
...
expressao_precedencia_2:
    ...
    | expressao_precedencia_2 '*' expressao_precedencia_1 {
        ...
        $$->code = iloc_list_concat(iloc_list_concat($1->code, $3->code), iloc_list_create_node(mult_op));
    }
    | expressao_precedencia_2 '/' expressao_precedencia_1 { 
        symbol_table_type_t type = infer_type($1->type, $3->type);
        $$ = asd_new("/", type); 
        ...
        $$->code = iloc_list_concat(iloc_list_concat($1->code, $3->code), iloc_list_create_node(div_op));
    }
    | expressao_precedencia_2 '%' expressao_precedencia_1 { 
        symbol_table_type_t type = infer_type($1->type, $3->type);
        $$ = asd_new("%", type); 
...
expressao_precedencia_1:
    ...
    | '-' expressao_precedencia_1 { 
        ...
        ILOCOperation *loadI_op = iloc_operation_create("loadI", "-1", loadReg, NULL , NULL);
